...
// @updateURL   https://github.com/zmnmxlntr/hg/raw/refs/heads/alt/alt.user.js
// @iconURL     https://github.com/zmnmxlntr/hg/raw/master/icon.png
// @include     /^(https?://)?lynx.farted.net/hg/res/*
...
(function() {
    if(window.location.hostname === "lynx.farted.net") {
        ...
        function hgDraw() {
            ...
            const threadPosts = document.getElementsByClassName("postCell");
            for(let i = 0; i < threadPosts.length; i++) {
                try {
                    if(threadPosts[i].getElementsByClassName(class_hgCheckbox).length === 0) {
                        const postNumber = threadPosts[i].id;
                        const postImage  = threadPosts[i].getElementsByClassName("imgLink");
                        ...
                        if(postImage.length) {
                            ...
                            let nom = threadPosts[i].getElementsByClassName("divMessage")[0].innerText.split('\n');
                            let female = false;
        ...
        function hgSave() {
            ...
            for(let i = 0, count = 0; i < tributeForms.length && count < hgReapingSize; i++) {
                if(tributeForms[i].getElementsByClassName(class_hgCheckbox)[0].checked === true) {
                    ...
                    imgsStr += tributeForms[i].parentElement.getElementsByClassName("imgLink")[0].href + "|";
            ...
            GM_setValue("nomsStr", nomsStr.slice(0, -1));
            GM_setValue("gensStr", gensStr.slice(0, -1));
        ...
        const hgUpcoming_div = hgCreateElement_Div("hgUpcoming-panel", "display:none;", "<br>Upcoming features and changes:<br>&nbsp;- Add support for Murder Games and the new version of Brantsteele's simulator<br>&nbsp;- Add locally modifiable list of autogrills<br>&nbsp;- Customize keybinds<br>&nbsp;- Retain edited forms through page refreshes<br>&nbsp;- Reset forms to original<br>&nbsp;- Retain page position when drawing new forms<br>&nbsp;- Additional code refactoring for the sake of maintainability and readability (not that you care)<br><br>For bugs/suggestions/questions/feedback, contact me on Discord: ZMNMXLNTR#6271<br>Alternatively, submit an issue to the <a href='https://github.com/zmnmxlntr/hg' target='_blank'>repository</a>.");
        ...
        function hgLoad(rolling=false) {
            ...
            // Populate all saved values into the reaping form. Blank any entry forms that do not yet have a corresponding saved value.
            for(i = 2, j = 0; i < inputs.length && j < hgReapingSize && j < capacity && j < imgs.length; i += 4, j++) {
                // ToDO: Check to make sure doing removeAttribute on an attribute that's not there doesn't break shit
            ...
            if(rolling === true) {
                for(null; i < inputs.length && j < hgReapingSize && j < capacity; i += 4, j++) {
                    inputs[i + 2].value = inputs[i].value = noms[j % noms.length];
                    inputs[i + 3].value = inputs[i + 1].value = imgs[j % noms.length];
                }
            } else {
            ...
            // Assign genders to all saved tributes. Set gender to '?' for any positions that have not yet been filled
            for(i = 1, j = 0; i < hgReapingSize * 3 + 1 && j < hgReapingSize && j < capacity && j < imgs.length; i += 3, j++) {
                genders[i].value = gens[j];
            }
            if(rolling === true) {
                for(null; i < hgReapingSize * 3 + 1 && j < hgReapingSize && j < capacity; i += 3, j++) {
                    genders[i].value = gens[j % gens.length];
                }
            } else {
