...
if(window.location.hostname === "boards.4chan.org" || window.location.hostname === "boards.4channel.org") {
    ...
    function hgSave() {
        ...
        for(let i = 0, count = 0; i < tributeForms.length && count < hgReapingSize; i++) {
            if(tributeForms[i].getElementsByClassName(class_hgCheckbox)[0].checked === true) {
                ...
                json.chars[i] = {};
                json.chars[i].name = tributeForms[i].getElementsByClassName(class_hgField)[0].value;
                json.chars[i].g = tributeForms[i].getElementsByClassName(class_hgGender)[0].checked === true ? "1" : "0"; // ToDO: Does not account for "they"
                json.chars[i].t = 0;
                json.chars[i].team = "NOTEAM";
                json.chars[i].perks = [];
                if(optFullImgs === true) {
                    json.chars[i].pic = tributeForms[i].parentElement.getElementsByClassName("fileThumb")[0].href;
                } else {
                    json.chars[i].pic = tributeForms[i].parentElement.getElementsByTagName("img")[0].src;
                }
    ...
    const grills_dict = [], grills_array = [
        ...
        'loona', 'loonacity', 'hop', 'marceline the vampire queen', 'marcy', 'roxy glamcock', 'ranma', 'vanellope von schweetz', 'glamcock chica', 'teto', 'erkle'
    ];
